...
public class MiniGame1Glove : MonoBehaviour
{
    [Header("Serial")]
    ...
    public float selectRearmSeconds = 0.75f;
    ...
    private void HandlePose(string pose)
    {
        ...
        switch (pose)
        {
            case "LEFT": game?.GloveMoveLeft(); consumed = true; Debug.Log("LEFT"); break;
            case "RIGHT": game?.GloveMoveRight(); consumed = true; Debug.Log("RIGHT"); break;
            case "UP": game?.GloveMoveUp(); consumed = true; Debug.Log("UP"); break;
            case "DOWN": game?.GloveMoveDown(); consumed = true; Debug.Log("DOWN"); break;
        }
    ...
    private void TryCompleteFistIfReady()
    {
        ...
        if (_fingersSeenThisWindow.Count == 5)
        {
            ...
            if (requireNeutralBetweenCommands) _requireNeutral = true;
            game?.GloveSelectOrSwapArm();
            ResetFistWindow();
        }
    }
