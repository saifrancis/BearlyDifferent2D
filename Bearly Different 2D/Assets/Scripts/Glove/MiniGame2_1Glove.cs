...
public class MiniGame2_1Glove : MonoBehaviour
{
    [Header("Serial")]
    public string portName = "COM3";
    public int baudRate = 9600;
    [Tooltip("Your Arduino prints finger names and optionally 'FLEX a b c d e' lines")]
    public bool expectsCsv = false; // not used here, but kept for parity 
    ...
    public BeehiveMiniGame1 beehiveGame; // assign in Inspector 
    ...
    // main-thread queue for serial thread -> Unity thread 
    ...
    {
        if (beehiveGame == null)
            Debug.LogWarning("[Glove] BeehiveMiniGame1 reference is not set. Assign it in the inspector.");
    ...
    private void AddFingerAndTryComplete(string finger)
    {
        ...
        {
            float now = Time.time;
            if (now - _lastFistTime >= fistCooldownSeconds)
            {
                _lastFistTime = now;
                // Trigger the beehive hit (same as pressing Space) 
                if (beehiveGame != null) beehiveGame.OnFist();
